#include "WeightNormSTDPPlasticity.hpp"

WeightNormSTDPPlasticity::WeightNormSTDPPlasticity(SpikingSynapses* synapses, SpikingNeurons* neurons, SpikingNeurons* input_neurons, plasticity_parameters_struct* parameters){
  
  plasticity_parameters = (weightnorm_stdp_plasticity_parameters_struct*) parameters;

}

WeightNormSTDPPlasticity::~WeightNormSTDPPlasticity(){
  free(sum_squared_afferent_values);
  free(afferent_weight_change_updater);
  free(neuron_in_plasticity_set);
  free(initial_weights);
}

void WeightNormSTDPPlasticity::state_update(unsigned int current_time_in_timesteps, float timestep){
  //if (((int)round(current_time_in_seconds / timestep) % 1000) == 0)
  backend()->weight_normalization();
}

void WeightNormSTDPPlasticity::reset_state() {
  for (int index = 0; index < total_number_of_plastic_synapses; index++){
      int synindex = plastic_synapses[index];
      initial_weights[index] = model->spiking_synapses->synaptic_efficacies_or_weights[synindex];
  }
  backend()->reset_state();
}


void WeightNormSTDPPlasticity::prepare_backend_early(){
  printf("Prepping\n");
  STDPPlasticity::prepare_backend_early();
  // By making use of the neuron and synapses, I can determine which weights are contributing to the calculation to be done
  if (plasticity_parameters) {
  if (plasticity_rule_id >= 0){
    sum_squared_afferent_values = (float*) malloc (model->spiking_neurons->total_number_of_neurons*sizeof(float));
    afferent_weight_change_updater = (float*) malloc (model->spiking_neurons->total_number_of_neurons*sizeof(float));
    initial_weights = (float*) malloc (total_number_of_plastic_synapses*sizeof(float));
    neuron_in_plasticity_set = (bool*) malloc (model->spiking_neurons->total_number_of_neurons*sizeof(bool));
    // Initialize the above values to -1, Only those neurons involved in the weight normalization rule should be counted.
    for (int neuronid = 0; neuronid < model->spiking_neurons->total_number_of_neurons; neuronid++){
      sum_squared_afferent_values[neuronid] = 0.0f;
      afferent_weight_change_updater[neuronid] = 0.0f; 
      neuron_in_plasticity_set[neuronid] = false;
    }
    // Now for every synapse (that is a part of this stdp rule), find the post-synaptic neuron and sum the weight^2
    int num_synapses = total_number_of_plastic_synapses;
    for (int index = 0; index < num_synapses; index++){
      int synindex = plastic_synapses[index];
      int postneuron = model->spiking_synapses->postsynaptic_neuron_indices[synindex];
      neuron_in_plasticity_set[postneuron] = true;
      sum_squared_afferent_values[postneuron] += pow(model->spiking_synapses->synaptic_efficacies_or_weights[synindex], 2.0f);
      initial_weights[index] = model->spiking_synapses->synaptic_efficacies_or_weights[synindex];
    }
    // If there is a target total, then meet it:
    if (plasticity_parameters->settarget){
      for (int index = 0; index < num_synapses; index++){
        int synindex = plastic_synapses[index];
        int postneuron = model->spiking_synapses->postsynaptic_neuron_indices[synindex];
        if (postneuron >= 0){
          model->spiking_synapses->synaptic_efficacies_or_weights[synindex] /= sqrt(sum_squared_afferent_values[postneuron]);
          model->spiking_synapses->synaptic_efficacies_or_weights[synindex] *= plasticity_parameters->target;
        }
      }
      for (int neuronindx = 0; neuronindx < model->spiking_neurons->total_number_of_neurons; neuronindx++){
        sum_squared_afferent_values[neuronindx] = plasticity_parameters->target;
      }
    }
  }
  }
}

SPIKE_MAKE_INIT_BACKEND(WeightNormSTDPPlasticity);
