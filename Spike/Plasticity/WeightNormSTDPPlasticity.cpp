...
void WeightNormSTDPPlasticity::prepare_backend_early(){
  ...
  if (syns && neurs && plasticity_parameters) {
		...
		plastic_weights = (float*) malloc (total_number_of_plastic_synapses*sizeof(float));
		...
		for (int synindex = 0; synindex < num_synapses; synindex++){
			int postneuron = syns->postsynaptic_neuron_indices[plastic_synapses[synindex]];
			neuron_in_plasticity_set[postneuron] = true;
			sum_squared_afferent_values[postneuron] += syns->synaptic_efficacies_or_weights[plastic_synapses[synindex]]; //powf(syns->synaptic_efficacies_or_weights[synindex], plasticity_parameters->norm_order);
		}
		...
		if (plasticity_parameters->settarget){
			for (int synindex = 0; synindex < num_synapses; synindex++){
				...
				if (postneuron >= 0){
					syns->synaptic_efficacies_or_weights[synindex] /= sum_squared_afferent_values[postneuron];//powf(sum_squared_afferent_values[postneuron], (1.0f / (float)(plasticity_parameters->norm_order)));
		...
		for (int synindex = 0; synindex < num_synapses; synindex++){
      plastic_weights[synindex] = syns->synaptic_efficacies_or_weights[plastic_synapses[synindex]];
    }
	}
}
