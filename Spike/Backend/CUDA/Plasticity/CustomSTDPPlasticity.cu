    ...
    __global__ void ltp_and_ltd
      ...
      while (indx < total_number_of_plastic_synapses) {
        ...
        if (pre_spike_g >= 0){
          ...
          syn_update_val -= stdp_vars.learning_rate * ((old_synaptic_weight / stdp_vars.w_max) * temp_post_trace + stdp_vars.a_star);
        }
        ...
        if (post_spike_g >= 0){
          ...
          syn_update_val += stdp_vars.learning_rate * (1.0f - (old_synaptic_weight / stdp_vars.w_max)) * temp_pre_trace;
