...
void RateActivityMonitor::state_update(unsigned int current_time_in_timesteps, float timestep) {
  backend()->add_spikes_to_per_neuron_spike_count(current_time_in_timesteps, timestep);
}

void RateActivityMonitor::final_update(unsigned int current_time_in_timesteps, float timestep){
